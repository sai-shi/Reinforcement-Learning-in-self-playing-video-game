"""
A version of Proximal Policy Optimization (PPO) using multi agents and simple neural network.
...
S_DIM = 88*80*1
env = gym.make(GAME)
A_DIM = env.action_space.n  # state and action dimension
...
def preprocess(obs):
    ...
    img = (img // 3 - 128).astype(np.int64)    # Normalization
...
class PPO(object):
    ...
    def update(self):
        ...
        self.saver.save(self.sess, 'models/nn_ai_models/test_model')

    # Neural net of predicting action
    ...
    # Get the predicted value from critic
    def get_v(self, s):
        if s.ndim < 2:
            s = s[np.newaxis, :]
            ...
            for t in range(EP_LEN):
                ...
                if t == EP_LEN - 1 or GLOBAL_UPDATE_COUNTER >= MIN_BATCH_SIZE or done:
                    if done:
                        ...
                        v_s_ = self.ppo.get_v(preprocess(s_))
...
if __name__ == '__main__':
    ...
    # Save the results
    with open('results/nn_ai.pickle', 'wb') as fp:
    ...
    # plt.ylabel('Reward')
    # plt.ion()
    # plt.savefig('nn_ai_learning.jpg')
    # plt.show()
    ...
    #         action = GLOBAL_PPO.choose_action(obs)
    #         time.sleep(0.01)
