"""
A version of Proximal Policy Optimization (PPO) using multi agents and LSTM-CNN.
...
from keras.layers import Conv2D, LSTM, Flatten, TimeDistributed, Dense
import numpy as np
...
GAME = 'MsPacman-v0'
seq = 4
S_DIM, A_DIM = 88 * 80 * 1 * seq, 9         # state and action dimension
...
def lstm_cnn(obs, trainable):
    x = tf.reshape(obs, [-1, 88, 80, 1, seq])
    lstm_cell = tf.contrib.rnn.LSTMCell(num_units=100)
    # initial_state = lstm_cell.zero_state(MIN_BATCH_SIZE, dtype=tf.float32)
...
class PPO(object):
    ...
    def update(self):
        ...
        self.saver.save(self.sess, 'models/lstm_cnn_ai_models/test_model')

    def _build_anet(self, name, trainable):
        with tf.variable_scope(name):
            la = lstm_cnn(self.tfs, trainable)
    ...
    def work(self):
        ...
        while not COORD.should_stop():
            ...
            for t in range(EP_LEN):
                ...
                a = self.ppo.choose_action(imgs_arr)
                s_, r, done, _ = self.env.step(a)
                if done:
                    r = -10
                buffer_s.append(imgs_arr)
                buffer_a.append(a)
...
if __name__ == '__main__':
    ...
    with open('results/lstm_cnn_ai.pickle', 'wb') as fp:
