"""
A version of Proximal Policy Optimization (PPO) using multi agents and pretrained CNN.
...
# def pretrain_model_actor(x, trainable):
#     # Reshape to [batch, height, width, channels].
#     obs = tf.reshape(x, [-1, 88, 80, 1])
#     # Adjust image size to Inception-v3 input.
#     imgs = tf.image.resize_images(obs, (32, 32))
#     input_layer = tf.image.grayscale_to_rgb(imgs)
#     # Load Inception-v3 model.
#     la = MODULE(input_layer)
#     actor = tf.layers.dense(la, A_DIM, tf.nn.softmax, trainable=trainable)
#
#     return actor


def pretrain_model(x, trainable):
    # Reshape to [batch, height, width, channels].
    obs = tf.reshape(x, [-1, 88, 80, 1])
    # Adjust image size to Inception-v3 input.
    imgs = tf.image.resize_images(obs, (32, 32))
...
class PPO(object):
    def __init__(self):
        ...
        # critic
        lc = pretrain_model(self.tfs, True)
        ...
        # self.sample_op = tf.argmax(tf.squeeze(pi.sample(1), axis=0))       # choosing action
    ...
    def update(self):
        ...
        self.saver.save(self.sess, 'models/pretrain_cnn_ai_models/test_model')

    def _build_anet(self, name, trainable):
        with tf.variable_scope(name):
            la = pretrain_model(self.tfs, trainable=trainable)
...
if __name__ == '__main__':
    ...
    with open('results/pretrain_cnn_ai.pickle', 'wb') as fp:
    ...
    # plt.ylabel('Reward')
    # plt.ion()
    # plt.savefig('pretrain_cnn_ai_learning.jpg')
    # plt.show()
    # env = gym.make(GAME)
    # total = 0
    # for i in range(100):
